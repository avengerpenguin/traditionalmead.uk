...
def fetch(fetch_url, name_from):
    print('Fetching {}...'.format(fetch_url), file=sys.stderr)
    ...
    for link in links:
        if link['href']:
            ...
            if name_to in [
                'disqus.com', 'www.pinterest.com', 'types-of-mead',
                'metheglin-spiced-mead', 'scottish-meads', 'english-meads',
                'cyser', 'craft-mead', 'pure-honey-mead', 'mead-liqueurs',
                'chouchen', 'sparkling', 'dry-meads', 'pyment', 'treacle',
                'christmas-meads',
            ]:
                continue
