...
token = graph.get_app_access_token('1405522322829296', os.getenv('FACEBOOK_CLIENT_TOKEN'))
...
if True or random.randint(3, 10) <= quiet_period.days:
    ...
    for url in not_posted:
        ...
        if og_type_meta_tag and og_type_meta_tag['content'] == 'article':
            ...
            if first_img_tag:
                article['image'] = urlparse.urljoin('http://traditionalmead.uk/', first_img_tag['src'])
    ...
    consumer = oauth.Consumer(key='599b51694409551417438196',
                              secret=os.getenv('BUFFER_CLIENT_SECRET'))
    request_token_url = 'https://bufferapp.com/oauth2/authorize'

    print(http.post('https://api.bufferapp.com/1/updates/create.json', data=[
